      ...
      const dummyActivities = [
        {
          id: 'user-1',
          type: 'user',
          message: 'New user registration: Abhishek Rajput',
          time: '2025-11-05 10:30 AM',
        },
        {
          id: 'order-1',
          type: 'order',
          message: 'New order placed - ₹1,299',
          time: '2025-11-05 09:45 AM',
        },
        {
          id: 'ticket-1',
          type: 'ticket',
          message: 'Support ticket opened by user1234',
          time: '2025-11-04 07:12 PM',
        },
        {
          id: 'server-1',
          type: 'server',
          message: 'New VPS instance activated',
          time: '2025-11-04 03:20 PM',
        },
      ];

      setStats(dummyStats);
      setRecentActivity(dummyActivities);
      setLoading(false);
    }, 1000);
        ...
        {statCards.map((stat, index) => {
          ...
          return (
            <Link
              key={index}
              to={stat.link}
              className={`bg-slate-900 p-6 rounded-xl shadow-sm border-2 ${colors.border} hover:shadow-lg transition group`}
            >
              <div className="flex items-center justify-between mb-4">
                <div
                  className={`w-14 h-14 ${colors.bg} rounded-xl flex items-center justify-center group-hover:scale-110 transition`}
                >
                  <stat.icon className={`h-7 w-7 ${colors.text}`} />
                </div>
                <TrendingUp className="h-5 w-5 text-slate-500 group-hover:text-cyan-400 transition" />
      ...
      <div className="bg-gradient-to-br from-slate-900 via-cyan-900 to-slate-900 rounded-xl shadow-lg p-8 text-white border-2 border-cyan-500">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-2xl font-bold mb-2">Platform Performance</h3>
            <p className="text-cyan-200 mb-4">
              Your hosting platform is running smoothly with {stats.totalUsers} users and{' '}
